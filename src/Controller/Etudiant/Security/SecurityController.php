...
class SecurityController extends AbstractController
{
    ...
    /**
     * @Route("/etudiant/login", name="app_login")
     */
    ...
    /**
     * @Route("/etudiant/inscription", name="etudiant_register", methods={"GET","POST"})
    ...
    ): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('etudiant_after_inscription');
        }
    ...
    /**
     * @Route("/etudiant/mot-de-passe-oubliee", name="etudiant_forgotten_password", methods={"GET","POST"})
     */
    ...
    ): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($user === null) {
                ...
                $url = $this->generateUrl('etudiant_reset_password', array('token' => $token), UrlGeneratorInterface::ABSOLUTE_URL);

                $sendEmail->resetPassword($url, $user);

                $this->addFlash('notice', 'Mail envoyé');

                return $this->redirectToRoute('etudiant_after_send_mail');
            }
        }
    ...
    /**
     * @Route("/etudiant/reinitialiser-mot-de-passe/{token}", name="etudiant_reset_password")
     */
    public function resetPassword(Request $request, string $token,
     UserPasswordEncoderInterface $passwordEncoder,
      GuardAuthenticatorHandler $guardAuthenticatorHandler,
      UserFormAuthenticator $loginFormAuthenticator)
    {
     ...
     /**
     * @Route("/etudiant/activate-compte/{token}", name="etudiant_activate_compte")
     */
    ...
    /**
     * @Route("/etudiant/logout", name="app_logout")
     */
